...
export const ThreeDMarquee: React.FC<ThreeDMarqueeProps> = ({
  ...
  return (
    <section
      className={`mx-auto block h-[500px] w-full max-w-none max-sm:h-[300px]
        overflow-hidden rounded-2xl bg-#07070D ${className}`}
    >
      <div
      ...
      >
        <div className="w-full overflow-hidden scale-90 sm:scale-100">
          <div
            className="relative grid h-full origin-center gap-0 transform"
          ...
          >
            {imageGroups.map((imagesInGroup, idx) => (
              ...
              >
                <div className="absolute left-0 top-0 h-full w-0.1 bg-gray-200 dark:bg-gray-700" />
                {imagesInGroup.map((image, imgIdx) => {
                  ...
                  return (
                    <div key={`img-${imgIdx}`} className="relative">
                      <div className="absolute top-0 left-0 w-100px h-0.5 bg-gray-200 dark:bg-gray-700" />
                      <motion.img
                        whileHover={{ y: -10 }}
                        ...
                        width={970}
                        height={700}
                        className={`aspect-[970/700] w-full max-w-none rounded-lg object-cover ring ring-gray-300/30 dark:ring-gray-800/50 shadow-xl hover:shadow-2xl transition-shadow duration-300 ${
